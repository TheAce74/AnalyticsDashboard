...
export default function OrdersTable({ stopIndex }) {
  ...
  const showModal = (id, name, date, amount, status) => {
    modalRef.current = {
      ...
      callback: () => showAlert("Download successful", { variant: "success" }),
    };
    setOpenModal(true);
  };
    ...
    <div className="orders-table">
      <table>
        ...
        <tbody>
          {users
            .slice(0, stopIndex)
            .map(({ image, name, date, amount, status, invoice }) => (
              <tr key={uuidv4()}>
                <td>
                  <div>
                    <img src={image} alt="" />
                ...
                <td>
                  <button
                    onClick={() =>
                      showModal(invoice, name, date, amount, status)
                    }
                  >
                    <svg
                      width="16"
